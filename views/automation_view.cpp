...
AutomationView::AutomationView(int w, int h, DeviceG1B* g1b,
                               DeviceRegloIcc* reglo_icc,
                               MonitorView* monitor_view, QWidget* parent)
    : w(w),
      h(h),
      h_delta_1(12),
      h_delta_2(32),
...
void AutomationView::InitLists() {
  process_name_list = {
      "Start pump channel 1 and 2", "Stop pump channel 2",
      "Start pulse generator",      "Start pump channel 2",
      "Stop pulse generator",       "Stop pump channel 1 and 2"};

  process_function_list = {&AutomationView::StartPumpAllChannel,
  ...
  parameter_name_list = {"rpm", "dir", "freq", "pw", "voltage", "offset"};

  parameter_function_list = {
      &AutomationView::GetRpm,     &AutomationView::GetDir,
      &AutomationView::GetFreq,    &AutomationView::GetPw,
      &AutomationView::GetVoltage, &AutomationView::GetOffset};
}
...
void AutomationView::SetupUi() {
  ...
  for (int i = 0; i < unit_cnt; i++) {
    ...
    process_unit_list[i] = new MetProcessUnit(process_unit_style, i, name, time,
                                              is_normal_unit, this);
  }
  ...
  all_process = new MetProcessUnit(process_unit_style, 0, "All process",
                                   "Total time:", false, this);
  all_process->time_edit->setText("");
  all_process->time_edit->setEnabled(false);
  all_process->label_name->setObjectName("all_process_name");
  all_process->label_name->setStyleSheet(
      "QLabel#all_process_name {"
      "background-color: " +
      QString(COLOR_BLACK) +
      ";"
      "border-radius: 5px;"
      ...
      ";"
      "}");
  all_process->time_edit->setStyleSheet(
      "MetLineEdit {"
  ...
  container_left->setStyleSheet(
      "QWidget#pulse_gen_container {"
      "background-color: " +
      QString(COLOR_LIGHT_GRAY) +
      ";"
      "border-radius: 10px;"
      "}");
  ...
  container_right->setStyleSheet(
      "QWidget#pump_container {"
      "background-color: " +
      QString(COLOR_LIGHT_GRAY) +
      ";"
      "border-radius: 10px;"
  ...
  tree =
      new MetTree(tree_style, (width() - 750) / 2, height() - h_delta_1, this);
  layout_left->addWidget(tree, 0, Qt::AlignTop);
  container_left->setLayout(layout_left);

  MetTextEditStyle text_style;
  text = new MetTextEdit(text_style, (width() - 750) / 2, (height() - 20) / 2,
                         this);
  canvas = new MetCanvas(monitor_view->helper, (width() - 750) / 2,
                         (height() - 20) / 2, this);
...
void AutomationView::LoadStyleSheet() {
  ...
  style_sheet_lcd =
      ...
      ";"
      "}";
  style_sheet_status_on = "background-color: " + QString(COLOR_ON_1) +
                          ";"
                          "border-radius: 2px;";
  style_sheet_status_off = "background-color: " + QString(COLOR_OFF_1) +
                           ";"
                           "border-radius: 2px;";
}
...
void AutomationView::mousePressEvent(QMouseEvent* event) {
  if (event->button() == Qt::RightButton) {
    ...
    QIcon icon_reset =
        QIcon(QString::fromStdString(GetAbsPath(IMAGE_MET_RESET)));
...
void AutomationView::Set() {
  ...
  while (parameter_name_list.size() > 0) {
    ...
    QList<QTreeWidgetItem*> list =
        tree->findItems(name, Qt::MatchContains | Qt::MatchRecursive, 0);
...
void AutomationView::RunProcess() {
  ...
  worker->Reset();
  thread->start();
  if (g_mode == Mode::DEBUG || g_mode == Mode::MONITOR_TEST) {
    ui_timer->start(10);
...
void AutomationView::StartPumpAllChannel() {
  ...
  if (g_mode == Mode::NORMAL || g_mode == Mode::DEBUG) {
...
void AutomationView::UpdateUi() { canvas->update(); }
...
void TimerWorker::Reset() { count = 0; }
...
void TimerWorker::Finish() { timer->stop(); }
