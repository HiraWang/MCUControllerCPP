...
MetComboBox::MetComboBox(MetComboBoxStyle style, int w, int h, QWidget* parent)
    : style(style), w(w), h(h), QComboBox(parent) {
  setFixedWidth(w);
  setFixedHeight(h);
  LoadStyleSheet();
...
MetComboBoxStyle::MetComboBoxStyle(
    QString bkg_color, QString selection_bkg_color, QString editable_color,
    QString disabled_color, QString hover_color, QString drop_down_color,
    QString down_arrow_color, QString down_arrow_on_color, QString font_size,
    QString font_color)
    : bkg_color(bkg_color),
