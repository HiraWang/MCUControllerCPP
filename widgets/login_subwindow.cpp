...
MetLoginSubwindow::MetLoginSubwindow(SerialPort* device, QString account,
                                     QString password, QWidget* parent)
    : device(device), account(account), password(password), QDialog(parent) {
  connect(this, SIGNAL(SignalLoginSubwindowFailed()), parent,
          SLOT(SetSerialStatusFail()));
  connect(this, SIGNAL(SignalLoginSubwindowFinished()), parent,
          SLOT(SetSerialStatusOk()));
  connect(this, SIGNAL(SignalLoginSubwindowFinished()), parent,
          SLOT(SetupUi()));
...
void MetLoginSubwindow::SetupUi() {
  ...
  MetLabel* account_label =
      new MetLabel(label_style, "Account:", 120, WIDGET_H, this);
  MetLabel* password_label =
      new MetLabel(label_style, "Password:", 120, WIDGET_H, this);
