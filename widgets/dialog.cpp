...
MetDialog::MetDialog(MetParaList* para_list, QWidget* parent)
    : para_list(para_list), QDialog(parent) {
...
void MetDialog::SetupUi() {
  ...
  setFixedWidth(300);
  QString style_sheet =
      QString("QDialog { background-color: ") + COLOR_GRAY + QString("; }");
  ...
  refresh_button =
      new MetButton(button_style, "refresh", "", BUTTON_W, BUTTON_W,
  ...
  for (int id = 0; id < para_list->size; id++) {
    ...
    AddAttribute(para_list->list[id], layout_list[id], line_edit_list[id],
                 para_list->list[id].is_editable, &para_count);
...
void MetDialog::AddAttribute(MetPara para, QHBoxLayout* layout,
                             MetLineEdit* line_edit, bool is_editable,
...
void MetDialog::UpdateAttributes() {
  for (int id = 0; id < para_list->size; id++) {
    ...
    if (para.str.empty()) {
      line_edit_list[id]->setText(
          QString::fromStdString(std::to_string(para.num)));
    } else {
...
void MetDialog::ToggleRefreshButton() {
  ...
  for (std::vector<std::string>::iterator i = ports.begin(); i != ports.end();
       ++i) {
    text->appendPlainText(QString::fromStdString(*i));
  }
}

void MetDialog::Show() { QDialog::show(); }

void MetDialog::Close() { QDialog::close(); }