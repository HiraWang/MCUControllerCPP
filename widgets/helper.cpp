...
Helper::Helper(HelperType type) : type(type) {
...
void Helper::SetExampleInfo(int elapsed) { this->elapsed = elapsed; }

void Helper::SetPulseChartInfo(int period, int pulse_width, int voltage,
                               int offset) {
  this->period = period;
...
void Helper::SetCount(size_t count) { this->count = count; }

void Helper::SetDataOffset(int data_offset) { this->data_offset = data_offset; }
...
void Helper::SetFirstRoundFlag(bool status) { this->is_first_round = status; }

void Helper::SetPlotInfoFlag(bool status) { this->show_plot_info = status; }

void Helper::SetScaleX(float scale_x) { this->scale_x = scale_x; }

void Helper::SetScaleY(float scale_y) { this->scale_y = scale_y; }

float Helper::GetScaleX() { return scale_x; }

float Helper::GetScaleY() { return scale_y; }
...
void Helper::paint(QPainter* painter, QPaintEvent* event, int period,
  ...
  painter->drawLine((int)pulse_width_f, voltage_t, (int)pulse_width_f,
                    voltage_max);
  ...
  for (int i = 0; i < info_count; i++) {
    ...
    info_list.pop_front();
    painter->drawText(
        QRect(5, i * text_box_height + 3, 150, (i + 1) * text_box_height),
        Qt::AlignLeft, info);
  }
...
void Helper::paint(QPainter* painter, QPaintEvent* event, size_t count) {
  ...
  painter->drawText(QRect(width_f - 65, height_f - 30, 50, 50), Qt::AlignRight,
                    QString::number((int)(data_count)));

  grid_font.setPixelSize(12 * scale_y);
  painter->setFont(grid_font);
  for (float i = 0.0f, j = 0.0f; i < height_f; i += height_interval, j++) {
    painter->drawLine(0, i, width, i);
    painter->drawText(QRect(5, i + 5, 50, 50), Qt::AlignLeft,
                      QString::number((grid_y_count - j) * 0.5f));
  }
