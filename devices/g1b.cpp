...
DeviceG1B::DeviceG1B(const wchar_t* port_name, DWORD baud_rate, BYTE byte_size,
...
SerialCode DeviceG1B::Login() {
  ...
  ret = LoginStepFunction("login step 1", std::string(), "avtech-f8369be5fff1",
                          5, 1000);
  if (ret == SERIAL_OK) {
...
SerialCode DeviceG1B::LoginStepFunction(std::string name, std::string input,
