...
DeviceRegloIcc::DeviceRegloIcc(const wchar_t* port_name, DWORD baud_rate,
...
SerialCode DeviceRegloIcc::Login() { return SERIAL_OK; }
...
SerialCode DeviceRegloIcc::SetRpm(BYTE channel, float rpm) {
  ...
  std::string buf =
      std::to_string(channel) + 'S' + rpm_i_stream.str() + rpm_f_stream.str();
  g_out << buf << '\n';
  buf += CR;
